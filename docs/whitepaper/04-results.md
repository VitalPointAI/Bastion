...
**Communication.** The swarm uses a BLE leader-spoke architecture. The leader (alpha, carrying the Jetson Orin Nano) drives headless follower platforms (bravo, charlie) directly over Bluetooth Low Energy, computing formation geometry and dispatching per-follower drive commands in parallel through the BLE Follower Manager. Follower positions are tracked via dead reckoning rather than external localization. This architecture ensures that swarm coordination does not depend on cloud connectivity — the leader carries mission parameters, formation logic, and follower control locally. If the BASTION API connection is interrupted, the leader continues commanding followers using cached mission state, maintaining formation integrity during degraded connectivity. A UDP peer mesh protocol (port 5807) is implemented for future expansion to compute-equipped peers that can participate as autonomous swarm members rather than commanded followers.
...
[^res17]: For foundational work on multi-robot coordination and leader-follower architectures, see Lynne E. Parker, "Multiple Mobile Robot Systems," in *Springer Handbook of Robotics*, ed. Bruno Siciliano and Oussama Khatib (Berlin: Springer, 2016), 1335-1384, https://doi.org/10.1007/978-3-319-32552-1_53.
