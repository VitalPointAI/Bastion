...
BASTION's swarm implementation belongs to the communicating swarm category, using a BLE leader-spoke architecture. A designated swarm leader robot (equipped with the Jetson Orin Nano) receives mission assignments from the DAO and translates them into formation commands distributed to follower platforms over Bluetooth Low Energy. Followers are headless — they carry no onboard compute — and the leader drives them directly, computing each follower's target position relative to its own using geometric formation models and dead-reckoned position tracking. This approach mirrors military doctrine's concept of movement formations: standardized spatial arrangements that provide specific tactical advantages.
...
BASTION's swarm communication uses a BLE leader-spoke architecture where the leader robot drives headless follower platforms directly over Bluetooth. The leader computes formation geometry, translates it into per-follower drive commands, and dispatches them in parallel through the BLE Follower Manager. Follower positions are tracked via dead reckoning rather than external localization. This architecture ensures that swarm coordination does not depend on cloud connectivity — the leader carries the mission parameters, formation logic, and follower control locally. If the BASTION API connection is interrupted, the leader continues commanding followers using cached mission state, maintaining formation integrity during periods of degraded connectivity. A UDP peer mesh protocol is implemented for future expansion to compute-equipped peers that can participate as autonomous swarm members rather than commanded followers.[^mesh2]

## 2.12.6 Neural Vision Pipelines at the Tactical Edge

Computer vision systems based on convolutional neural networks (CNNs) have achieved human-level performance on object detection and classification benchmarks, enabling autonomous threat recognition without continuous human supervision.[^vis1] BASTION's robot vision pipeline uses the YOLOv8 nano model (via the Ultralytics framework) running on the NVIDIA Jetson Orin Nano to process live camera feeds, detecting and classifying objects against trained label sets and returning bounding box coordinates and confidence scores in real time.[^vis1a] The Jetson Orin Nano's 67 TOPS of AI performance enables real-time inference at the edge without cloud connectivity, supporting the DDIL resilience requirements of tactical autonomous platforms.
...
[^vis1a]: Glenn Jocher, Ayush Chaurasia, and Jing Qiu, "Ultralytics YOLOv8," Ultralytics, 2023, https://github.com/ultralytics/ultralytics.
