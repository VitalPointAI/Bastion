...
Create basic CSS files for the components (LoginPage.css, PasskeySetup.css) following the existing military-themed styling patterns from the project.
  ...
  <verify>
Check files exist: `ls -la frontend/src/hooks/useAuth.ts frontend/src/components/LoginPage.tsx frontend/src/components/PasskeySetup.tsx`
Run `cd frontend && npx tsc --noEmit` to check TypeScript
  </verify>
  <done>
useAuth hook provides auth state management replacing usePrivy, LoginPage offers passkey and magic link options, PasskeySetup handles passkey registration with PRF support detection.
  </done>
</task>
...
function AuthContent({ children }: AuthWrapperProps) {
  ...
  const userContextValue = {
    ...
    accountId,
    email,
    mpcRegistered: false, // MPC not used with passkeys
    isAuthenticated,
  };
