...
must_haves:
  truths:
    ...
    - "Existing blinded-keys.ts remains backward compatible"
  artifacts:
  ...
  key_links:
    ...
    - from: "backend/src/auth/prf-did-integration.ts"
      ...
      via: "key derivation"
      pattern: "deriveDIDBlindedKey|deriveEncryptionKey"
---
...
import { deriveDIDBlindedKey, deriveEncryptionKey, blindedKeyToHex } from '../identity/blinded-keys.js';
 ...
 *
 * @param accountId - NEAR implicit account ID (from passkey public key)
 ...
 *
 * Separated from blinded key derivation for security.
 * Uses different domain separation.
 *
 * @param userSecret - User's secret (from NEAR key or PRF)
 * @returns 32-byte encryption key
 */
export function deriveEncryptionKey(userSecret: Uint8Array): Uint8Array {
  const salt = utf8ToBytes('bastion-encryption');
  const info = utf8ToBytes('did-encryption-v1');
...
IMPORTANT: Check if deriveEncryptionKey already exists in a different file (did-encryption.ts). If so, ensure consistency or add re-export.
  </action>
  <verify>
Run TypeScript compilation: `cd backend && npx tsc --noEmit`
Verify new function exists: `grep "derivePRFUserSecret\|deriveEncryptionKey" backend/src/identity/blinded-keys.ts`
  </verify>
  <done>
blinded-keys.ts updated with derivePRFUserSecret and deriveEncryptionKey (if not already present). Existing functions unchanged - full backward compatibility maintained.
  </done>
</task>
...
- deriveEncryptionKey exported from blinded-keys.ts
