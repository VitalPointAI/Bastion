...
must_haves:
  ...
  key_links:
    ...
    - from: "backend/src/index.ts"
      to: "backend/src/collaboration/sync-server.ts"
      via: "WebSocket server mount"
      pattern: "createSyncServer\\(server"
---
...
<task type="auto">
  <name>Task 3: Mount Collaboration WebSocket Server to Backend</name>
  <files>
    backend/src/index.ts
  </files>
  <action>
Integrate the Yjs collaboration WebSocket server with the Express backend.
...
2. After the existing WebSocket setup calls (after line ~103 where `setupOrchestrationWebSocket(server)` is called), add:
```typescript
...
The pattern follows existing WebSocket integrations:
- `setupMessageWebSocket(server)` - messaging
- `setupOrchestrationWebSocket(server)` - orchestration
- `createSyncServer(server, '/ws/collab')` - collaboration (new)

This mounts the Yjs WebSocket sync server at the `/ws/collab` path. Frontend clients will connect to `ws://localhost:3001/ws/collab?documentId=xxx&planId=xxx&did=xxx&name=xxx&role=xxx`.
  </action>
  <verify>
    - TypeScript compiles: `cd backend && pnpm exec tsc --noEmit`
    - Backend starts without errors: `cd backend && pnpm dev` (check logs for "Collaboration WebSocket server mounted")
    - WebSocket endpoint accessible: Connect to `ws://localhost:3001/ws/collab` (should reject without params but prove endpoint exists)
  </verify>
  <done>
    - createSyncServer imported from collaboration module
    - WebSocket server mounted at /ws/collab path
    - Console log confirms mount on startup
    - Frontend can connect to real-time collaboration endpoint
  </done>
</task>
...
6. WebSocket server mounted in backend/src/index.ts at /ws/collab
7. Backend starts and logs "Collaboration WebSocket server mounted at /ws/collab"
</verification>
...
- WebSocket endpoint /ws/collab available on backend
</success_criteria>
