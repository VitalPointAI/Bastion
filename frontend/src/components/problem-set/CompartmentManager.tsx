 ...
 * CompartmentManager
 *
 * Compartment CRUD and member assignment for problem set administrators.
 * Only visible to members with manage_problem set or manage_members permission
...
interface CompartmentCardProps {
  compartment: ProblemSetCompartment & { memberDids: string[] };
  allMembers: ProblemSetMemberDetail[];
  problemSetId: string;
  userDID: string;
  onChanged: () => void;
}
...
function CompartmentCard({
  ...
  async function handleAssign() {
    ...
    setActionError(null);
    try {
      await problemSetService.assignMemberToCompartment(
        problemSetId,
        compartment.id,
        selectedMemberDid,
        userDID,
  ...
  async function handleRemove(memberDid: string) {
    ...
    setActionError(null);
    try {
      await problemSetService.removeMemberFromCompartment(
        problemSetId,
        compartment.id,
        memberDid,
        userDID,
...
export function CompartmentManager({ problemSetId }: CompartmentManagerProps) {
  const { userDID } = useUser();
  const { userRoleInActive } = useProblemSet();

  const [compartments, setCompartments] = useState<
    Array<ProblemSetCompartment & { memberDids: string[] }>
  >([]);
  const [allMembers, setAllMembers] = useState<ProblemSetMemberDetail[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [comps, members] = await Promise.all([
        problemSetService.listCompartments(problemSetId, userDID),
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreateError(null);
    try {
      await problemSetService.createCompartment(
        problemSetId,
        newName.trim(),
  ...
  async function handleDelete(compartmentId: string, name: string) {
    ...
    try {
      await problemSetService.deleteCompartment(problemSetId, compartmentId, userDID);
      await loadData();
    } catch (err: unknown) {
