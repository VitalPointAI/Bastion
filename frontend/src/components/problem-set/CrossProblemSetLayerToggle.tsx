/**
 * CrossProblemSetLayerToggle
 *
 * Panel showing approved cross-problem set subscriptions as toggle switches.
 * Allows commanders to show/hide data layers from subscribed problem sets
 * on the overview map/panel.
 *
 * - Loads subscriptions from problem-set-service on mount
 * - Filters to approved subscriptions where this problem set is the subscriber
 * - Renders each as a toggle row with problem set name and data type badges
...
interface CrossProblemSetLayerToggleProps {
  problemSetId: string;
...
export function CrossProblemSetLayerToggle({
  problemSetId,
  onLayersChange,
}: CrossProblemSetLayerToggleProps) {
  ...
  const loadSubscriptions = useCallback(async () => {
    if (!problemSetId || !userDID) return;
    setLoading(true);
    setError(null);
    try {
      const result = await problemSetService.getSubscriptions(problemSetId, userDID);
      ...
      setEnabledLayers(new Set(approved.map((s) => s.publisherProblemSetId)));
    } catch {
  ...
  function handleToggle(publisherProblemSetId: string) {
    setEnabledLayers((prev) => {
      const next = new Set(prev);
      if (next.has(publisherProblemSetId)) {
        next.delete(publisherProblemSetId);
      } else {
        next.add(publisherProblemSetId);
      }
      return next;
    });
  }
    ...
    <div className="bg-gray-800 border border-gray-700 rounded-lg p-3 mb-4">
      ...
      {!loading && !error && subscriptions.length === 0 && (
        <p className="text-xs text-gray-500 py-2 leading-relaxed">
          No cross-problem set subscriptions. Request access from problem set commanders.
        </p>
      )}
      ...
      {!loading && !error && subscriptions.length > 0 && (
        <div className="max-h-48 overflow-auto space-y-0">
          {subscriptions.map((sub) => (
            ...
            >
              {/* Left: problem set name + data type badges */}
              ...
              <label className="relative inline-flex items-center cursor-pointer shrink-0">
                <input
                  type="checkbox"
                  checked={enabledLayers.has(sub.publisherProblemSetId)}
                  onChange={() => handleToggle(sub.publisherProblemSetId)}
                  className="sr-only peer"
                  aria-label={`Toggle ${sub.publisherProblemSetId} data layer`}
                />
                <div
                  className={[
                    'w-9 h-5 rounded-full transition-colors',
                    enabledLayers.has(sub.publisherProblemSetId)
                      ? 'bg-blue-600'
                      : 'bg-gray-600',
...
export default CrossProblemSetLayerToggle;
