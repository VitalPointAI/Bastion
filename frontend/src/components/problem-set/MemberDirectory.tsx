 ...
 * MemberDirectory
 *
 * Compartment-filtered member list for problem set members.
...
export function MemberDirectory({ problemSetId }: MemberDirectoryProps) {
  const { userDID } = useUser();
  const { userRoleInActive } = useProblemSet();

  const [members, setMembers] = useState<ProblemSetMemberDetail[]>([]);
  const [compartments, setCompartments] = useState<ProblemSetCompartment[]>([]);
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      const [allMembers, allCompartments, myCompIds] = await Promise.all([
        ...
        problemSetService.listCompartments(problemSetId, userDID).catch(() => [] as ProblemSetCompartment[]),
        problemSetService.getMyCompartments(problemSetId, userDID).catch(() => [] as string[]),
      ]);
      ...
      await Promise.all(
        allMembers.map(async (m) => {
          const cids = await problemSetService
            .getMyCompartments(problemSetId, m.userDid)
            .catch(() => [] as string[]);
