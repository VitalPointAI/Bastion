 ...
 *
 * Modal for creating workspace invite links and managing pending invites.
 * - Role selection from workspace roles
...
export function WorkspaceInviteModal({
  ...
  const loadRoles = useCallback(async () => {
    if (!userDID) return;
    try {
      const result = await workspaceService.listRoles(workspaceId, userDID);
      setRoles(result);
      if (result.length > 0) {
  ...
  const loadPendingInvites = useCallback(async () => {
    ...
    try {
      const result = await workspaceService.listPendingInvites(workspaceId, userDID);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { rawToken } = await workspaceService.createInvite(
        workspaceId,
      ...
      const link = `${window.location.origin}/workspace/invite/${rawToken}`;
      setCreatedLink(link);
      void loadPendingInvites();
    } catch (err) {
          ...
          {!createdLink ? (
            <form onSubmit={(e) => { void handleSubmit(e); }} className="space-y-5">
              ...
              <div>
                ...
                {roles.length === 0 ? (
                  <p className="text-sm text-gray-500">No roles configured for this workspace.</p>
                ) : (
                  <select
