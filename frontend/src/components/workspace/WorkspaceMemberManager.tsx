 ...
 *
 * Member management component for workspace admins.
 * - Lists all workspace members with role, status, join date
...
type ConfirmAction =
  | { type: 'change_role'; member: WorkspaceMemberDetail; roleId: string }
  | { type: 'suspend'; member: WorkspaceMemberDetail }
  | { type: 'unsuspend'; member: WorkspaceMemberDetail }
  | { type: 'remove'; member: WorkspaceMemberDetail };
...
export function WorkspaceMemberManager({ workspaceId }: WorkspaceMemberManagerProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [memberResult, roleResult] = await Promise.all([
        ...
        workspaceService.listRoles(workspaceId, userDID),
      ]);
  ...
  const handleConfirmedAction = async () => {
    ...
    try {
      if (confirm.type === 'change_role') {
        ...
        await workspaceService.suspendMember(workspaceId, member.userDid, userDID);
      } else if (confirm.type === 'unsuspend') {
        await workspaceService.unsuspendMember(workspaceId, member.userDid, userDID);
      } else if (confirm.type === 'remove') {
        await workspaceService.removeMember(workspaceId, member.userDid, userDID);
      }
      setConfirm(null);
      await loadData();
    } catch (err) {
...
function ConfirmDialog({ action, roles, onConfirm, onCancel, loading }: ConfirmDialogProps) {
  ...
  if (action.type === 'change_role') {
    ...
    body = `Suspend ${did}? They will lose access to the workspace until unsuspended.`;
    ...
    title = 'Remove Member';
    body = `Remove ${did} from the workspace? This action cannot be undone.`;
    confirmLabel = 'Remove';
