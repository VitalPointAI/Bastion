 ...
 * Shows outgoing subscriptions (this workspace as subscriber) and
 * incoming subscriptions (this workspace as publisher).
 *
...
import { workspaceService, type Subscription } from '../../lib/workspace-service';
...
export function SubscriptionManager({ workspaceId }: SubscriptionManagerProps) {
  const { memberships, userRoleInActive } = useWorkspace();
  ...
  // Available workspaces for subscription creation (exclude current workspace)
  const availableWorkspaces = memberships.filter(
  ...
  const loadSubscriptions = useCallback(async () => {
    ...
    setError(null);
    try {
      const { asSubscriber, asPublisher } = await workspaceService.getSubscriptions(
        workspaceId,
        userDID
      );
  ...
  const handleApprove = async (subId: string) => {
    ...
    setActionError(null);
    try {
      await workspaceService.updateSubscriptionStatus(workspaceId, subId, 'approved', userDID);
  ...
  const handleReject = async (subId: string) => {
    ...
    setActionError(null);
    try {
      await workspaceService.updateSubscriptionStatus(workspaceId, subId, 'rejected', userDID);
      await loadSubscriptions();
    } catch (err) {
  ...
  const handleUnsubscribe = async (subId: string) => {
    ...
    setActionError(null);
    try {
      await workspaceService.deleteSubscription(workspaceId, subId, userDID);
      await loadSubscriptions();
    } catch (err) {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setActionError(null);
    try {
      await workspaceService.createSubscription(
        workspaceId,
        selectedPublisherId,
        selectedDataTypes,
        userDID
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-lg p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Manage cross-workspace intelligence subscriptions.
          </p>
        </div>
        ...
        >
          ...
          <div>
            ...
            {availableWorkspaces.length === 0 ? (
              <p className="text-sm text-gray-500 italic">No other workspaces available.</p>
            ) : (
              <select
      ...
      <div>
        ...
        {outgoing.length === 0 ? (
          renderEmptyState('No outgoing subscriptions. Request access to another workspace\'s data above.')
        ) : (
          <div className="space-y-2">
            {outgoing.map((sub) => {
              const publisherName = memberships.find(
                (m) => m.workspaceId === sub.publisherWorkspaceId
              )?.name ?? sub.publisherWorkspaceId;
      ...
      <div>
        <h3 className="text-sm font-semibold text-gray-300 mb-3 flex items-center gap-2">
          ...
          Incoming Requests
          <span className="text-xs font-normal text-gray-500">
            (others requesting your workspace's data)
          </span>
        </h3>
          ...
          <div className="space-y-2">
            {incoming.map((sub) => {
              const subscriberName = memberships.find(
                (m) => m.workspaceId === sub.subscriberWorkspaceId
              )?.name ?? sub.subscriberWorkspaceId;
