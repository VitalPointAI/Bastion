 ...
 * Shows cross-workspace updates for that specific tab, allowing
 * commanders to take action without navigating away from the current tab.
 *
 * Features:
 * - Filters CrossWorkspaceUpdate[] to the relevant tab
 ...
 *
 * Phase 20 Plan 07: Tab notification dropdown for cross-workspace updates
 */
...
interface TabNotificationDropdownProps {
  tab: string;
  updates: CrossWorkspaceUpdate[];
  onClose: () => void;
  onAction: (update: CrossWorkspaceUpdate) => void;
}

// ─── Helpers ──────────────────────────────────────────────────────────────────

/** Badge styling for each update type */
const UPDATE_TYPE_STYLES: Record<CrossWorkspaceUpdate['updateType'], { label: string; className: string }> = {
...
export function TabNotificationDropdown({
  ...
  const { clearTabNotifications } = useWorkspace();
  ...
  function handleItemClick(update: CrossWorkspaceUpdate) {
    onAction(update);
  }
  ...
  return (
    <div
      ...
      role="menu"
      aria-label={`Cross-workspace updates for ${tab}`}
    >
      {/* Header */}
