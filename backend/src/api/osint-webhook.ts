...
osintWebhookRouter.delete('/feeds/:feedId', async (req, res) => {
  try {
    ...
    // Get feed info before deleting (need sourceName to clean up COP layers + events)
    const feed = await osintFeedStore.getFeed(feedId);
    if (!feed) {
      res.status(404).json({ error: 'Feed not found' });
      return;
    }

    // Delete the feed config
    ...
    try {
      ...
      feedPoller.stopFeedTimer(feedId);
    } catch { /* non-fatal */ }

    // Clean up events from this feed
    try {
      const pool = (await import('../lib/database.js')).getPool();
      await pool.query(
        `DELETE FROM osint_events WHERE source_name = $1 AND workspace_id = $2`,
        [feed.sourceName, feed.problemSetId],
      );
    } catch { /* non-fatal */ }

    // Clean up COP layers generated by this feed's events
    try {
      const pool = (await import('../lib/database.js')).getPool();
      await pool.query(
        `DELETE FROM cop_layers WHERE spec::text LIKE $1`,
        [`%${feed.sourceName}%`],
      );
    } catch { /* non-fatal */ }

    // Clean up graph nodes from this feed
    try {
      const { executeWriteQuery } = await import('../graph/neo4j-client.js');
      await executeWriteQuery(`
        MATCH (a:Actor)
        WHERE a.attributes IS NOT NULL
          AND a.attributes CONTAINS $sourceName
          AND (a.id STARTS WITH 'OSINT-' OR a.id STARTS WITH 'ACT-osint-')
        DETACH DELETE a
      `, { sourceName: feed.sourceName });
    } catch { /* non-fatal */ }

    console.log(`[OSINT Feeds] Deleted feed "${feed.sourceName}" and cleaned up events/layers/graph`);
    res.json({ success: true, cleaned: feed.sourceName });
  } catch (error) {
