/**
 * Problem Set Compartment Store
 *
 * Phase 23: Problem Set Model & Workspace Rename
 *
 * Manages problem_set_compartments and problem_set_member_compartments tables.
 * Compartments enforce need-to-know access beyond classification level.
 *
 * Example compartment names: 'SIGINT', 'HUMINT', 'OP-PLAN-X', 'CYBER'
 */

import { randomUUID } from 'crypto';
import { getPool } from '../lib/database.js';
import type { ProblemSetCompartment } from './types.js';

// --- Table Initialization ---

async function initCompartmentTables(): Promise<void> {
  const pool = getPool();
  await pool.query(`
    CREATE TABLE IF NOT EXISTS problem_set_compartments (
      id TEXT PRIMARY KEY,
      problem_set_id TEXT NOT NULL REFERENCES problem_sets(id) ON DELETE CASCADE,
      name TEXT NOT NULL,
      description TEXT,
      created_by TEXT NOT NULL,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      UNIQUE(problem_set_id, name)
    );
    CREATE INDEX IF NOT EXISTS idx_pc_problem_set ON problem_set_compartments(problem_set_id);

    CREATE TABLE IF NOT EXISTS problem_set_member_compartments (
      id TEXT PRIMARY KEY,
      problem_set_id TEXT NOT NULL REFERENCES problem_sets(id) ON DELETE CASCADE,
      member_did TEXT NOT NULL,
      compartment_id TEXT NOT NULL REFERENCES problem_set_compartments(id) ON DELETE CASCADE,
      assigned_by TEXT NOT NULL,
      assigned_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      UNIQUE(problem_set_id, member_did, compartment_id)
    );
    CREATE INDEX IF NOT EXISTS idx_pmc_problem_set ON problem_set_member_compartments(problem_set_id);
    CREATE INDEX IF NOT EXISTS idx_pmc_member ON problem_set_member_compartments(member_did);
    CREATE INDEX IF NOT EXISTS idx_pmc_compartment ON problem_set_member_compartments(compartment_id);
  `);
}

// --- Row mappers ---

interface CompartmentRow {
  id: string;
  problem_set_id: string;
  name: string;
  description: string | null;
  created_by: string;
  created_at: Date;
}

function mapCompartment(row: CompartmentRow): ProblemSetCompartment {
  return {
    id: row.id,
    problemSetId: row.problem_set_id,
    name: row.name,
    description: row.description,
    createdBy: row.created_by,
    createdAt: row.created_at,
  };
}

// --- Store Class ---

export class ProblemSetCompartmentStore {
  private initialized = false;

  private async ensureInitialized(): Promise<void> {
    if (!this.initialized) {
      await initCompartmentTables();
      this.initialized = true;
    }
  }

  /**
   * Create a new compartment in a problem set.
   * Compartment names must be unique per problem set.
   */
  async createCompartment(
    problemSetId: string,
    name: string,
    description: string | null,
    createdBy: string,
  ): Promise<ProblemSetCompartment> {
    await this.ensureInitialized();
    const pool = getPool();
    const id = `PC-${randomUUID()}`;

    const result = await pool.query<CompartmentRow>(
      `INSERT INTO problem_set_compartments (id, problem_set_id, name, description, created_by)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [id, problemSetId, name.trim().toUpperCase(), description, createdBy],
    );

    return mapCompartment(result.rows[0]);
  }

  /**
   * List all compartments for a problem set.
   */
  async listCompartments(problemSetId: string): Promise<ProblemSetCompartment[]> {
    await this.ensureInitialized();
    const pool = getPool();

    const result = await pool.query<CompartmentRow>(
      `SELECT * FROM problem_set_compartments WHERE problem_set_id = $1 ORDER BY name ASC`,
      [problemSetId],
    );

    return result.rows.map(mapCompartment);
  }

  /**
   * Get a compartment by ID.
   */
  async getCompartment(compartmentId: string): Promise<ProblemSetCompartment | null> {
    await this.ensureInitialized();
    const pool = getPool();

    const result = await pool.query<CompartmentRow>(
      `SELECT * FROM problem_set_compartments WHERE id = $1`,
      [compartmentId],
    );

    return result.rows[0] ? mapCompartment(result.rows[0]) : null;
  }

  /**
   * Delete a compartment and all its member assignments (via CASCADE).
   */
  async deleteCompartment(compartmentId: string): Promise<void> {
    await this.ensureInitialized();
    const pool = getPool();

    await pool.query(
      `DELETE FROM problem_set_compartments WHERE id = $1`,
      [compartmentId],
    );
  }

  /**
   * Assign a member to a compartment.
   * Idempotent — duplicate assignments are silently ignored.
   */
  async assignMember(
    problemSetId: string,
    memberDid: string,
    compartmentId: string,
    assignedBy: string,
  ): Promise<void> {
    await this.ensureInitialized();
    const pool = getPool();
    const id = `PMC-${randomUUID()}`;

    await pool.query(
      `INSERT INTO problem_set_member_compartments
         (id, problem_set_id, member_did, compartment_id, assigned_by)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (problem_set_id, member_did, compartment_id) DO NOTHING`,
      [id, problemSetId, memberDid, compartmentId, assignedBy],
    );
  }

  /**
   * Remove a member from a compartment.
   */
  async removeMember(
    problemSetId: string,
    memberDid: string,
    compartmentId: string,
  ): Promise<void> {
    await this.ensureInitialized();
    const pool = getPool();

    await pool.query(
      `DELETE FROM problem_set_member_compartments
       WHERE problem_set_id = $1 AND member_did = $2 AND compartment_id = $3`,
      [problemSetId, memberDid, compartmentId],
    );
  }

  /**
   * List all members assigned to a specific compartment.
   * Returns DIDs of members in the compartment.
   */
  async listMembersInCompartment(compartmentId: string): Promise<string[]> {
    await this.ensureInitialized();
    const pool = getPool();

    const result = await pool.query<{ member_did: string }>(
      `SELECT member_did FROM problem_set_member_compartments
       WHERE compartment_id = $1
       ORDER BY assigned_at ASC`,
      [compartmentId],
    );

    return result.rows.map((r) => r.member_did);
  }

  /**
   * List all compartment IDs that a member belongs to within a problem set.
   */
  async listCompartmentsForMember(
    problemSetId: string,
    memberDid: string,
  ): Promise<string[]> {
    await this.ensureInitialized();
    const pool = getPool();

    const result = await pool.query<{ compartment_id: string }>(
      `SELECT compartment_id FROM problem_set_member_compartments
       WHERE problem_set_id = $1 AND member_did = $2`,
      [problemSetId, memberDid],
    );

    return result.rows.map((r) => r.compartment_id);
  }

  /**
   * List compartments with their member DIDs for a problem set.
   * Useful for CompartmentManager display without extra round-trips.
   */
  async listCompartmentsWithMembers(
    problemSetId: string,
  ): Promise<Array<ProblemSetCompartment & { memberDids: string[] }>> {
    await this.ensureInitialized();
    const compartments = await this.listCompartments(problemSetId);

    const withMembers = await Promise.all(
      compartments.map(async (c) => {
        const memberDids = await this.listMembersInCompartment(c.id);
        return { ...c, memberDids };
      }),
    );

    return withMembers;
  }
}

export const problemSetCompartmentStore = new ProblemSetCompartmentStore();
