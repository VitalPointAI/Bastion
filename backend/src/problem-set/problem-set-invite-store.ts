...
export class ProblemSetInviteStore {
  private initialized = false;

  private async ensureInitialized(): Promise<void> {
    if (!this.initialized) {
      await initProblemSetInviteTable();
  ...
  ): Promise<{ invite: ProblemSetInvite; rawToken: string }> {
   ...
   */
  async getInviteByToken(rawToken: string): Promise<ProblemSetInvite | null> {
    await this.ensureInitialized();
    const pool = getPool();
    const hashedToken = createHash('sha256').update(rawToken).digest('hex');
   ...
   */
  async getInviteById(id: string): Promise<ProblemSetInvite | null> {
    await this.ensureInitialized();
    const pool = getPool();
  ...
  /**
   * List all invites for a problem set, ordered by creation date (newest first).
   */
  async listInvitesForProblemSet(problemSetId: string): Promise<ProblemSetInvite[]> {
    await this.ensureInitialized();
    const pool = getPool();
    const result = await pool.query(
  ...
  /**
   * List pending invites (not yet accepted and not expired) for a problem set.
   */
  async listPendingInvites(problemSetId: string): Promise<ProblemSetInvite[]> {
    await this.ensureInitialized();
    const pool = getPool();
    const result = await pool.query(
      `SELECT * FROM problem_set_invites
  ...
  /**
   * Mark an invite as approved by an admin (for gated problem sets).
   */
  ...
  /**
   * Map a database row (snake_case) to a ProblemSetInvite (camelCase).
   */
  private mapRow(row: {
    id: string;
...
export const problemSetInviteStore = new ProblemSetInviteStore();
